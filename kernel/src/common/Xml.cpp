...
/*    STRING <attrs> SLASH CLOSE OPEN
    ...
    */
    ...
    if (token.type == XmlToken::Slash) {
        ...
        if (strcmp(token.text.data(), tok.text.data()) != 0) { 
            Log("%d:%d: Mismatching name '%s' in close tag. Matched with '%s' open tag at %d:%d", line, chr, token.text.data(), tok.line, tok.chr, tok.text.data()); 
            delete node; 
            return NULL; 
        }
    } else {
...
XmlNode *XmlRead(const char *file, size_t length) {
    ...
    token = XmlTok(file, length, index, line, chr);
    if (token.type != XmlToken::String) { Log("%d:%d: Expected string", line, chr); delete node; return NULL; }
    if (strcmp(token.text.data(), openName.text.data()) != 0) { 
        Log("%d:%d: Mismatching name '%s' in close tag. Matched with '%s' open tag at %d:%d", line, chr, token.text.data(), openName.line, openName.chr, openName.text.data()); 
        delete node; 
        return NULL; 
    }
    token = XmlTok(file, length, index, line, chr);
...
void XmlNode::write(unsigned char *buffer, size_t &len, int depth) {
    if (buffer) {
        sprintf((char *)buffer, "%s<%s", (spacebuf + 49 - depth), type.data());
        buffer += strlen((char *)buffer);
        for (std::pair<rodvin::string, rodvin::string> arg : attributes) {
            sprintf((char *)buffer, " %s=\"%s\"", arg.first.data(), arg.second.data());
            buffer += strlen((char *)buffer);
        }
        if (children.size() == 0) {
            sprintf((char *)buffer, " />\n");
